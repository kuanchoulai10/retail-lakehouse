...
@patch("api.dependencies.k8s.k8s_config.load_incluster_config")
...
@patch(
    "api.dependencies.k8s.k8s_config.load_incluster_config",
    side_effect=__import__("kubernetes").config.config_exception.ConfigException(
        "not in cluster"
    ),
)
@patch("api.dependencies.k8s.k8s_config.load_kube_config")
