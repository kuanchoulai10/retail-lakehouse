"""Guard tests for application service naming conventions and UseCase compliance.

Rules enforced:
  1. Each service .py file must define a class ending in "Service"
...
def _service_modules() -> list[tuple[str, Path]]:
    """Return (dotted module path, file path) for all service .py files."""
...
def _service_id(param: tuple[str, Path]) -> str:
    """Format a service entry as a readable pytest ID."""
    _module, path = param
...
@pytest.mark.parametrize("service_entry", _service_modules(), ids=_service_id)
