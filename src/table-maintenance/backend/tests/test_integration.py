...
def test_full_crud_lifecycle():
    ...
    # 2. Create first job
    resp = client.post("/jobs", json=REWRITE_PAYLOAD)
    ...
    resp = client.get(f"/jobs/{name1}")
    assert resp.status_code == 200
    assert resp.json()["name"] == name1

    # 5. Create second job (different type)
    resp = client.post("/jobs", json=EXPIRE_PAYLOAD)
    assert resp.status_code == 201
    job2 = resp.json()
    name2 = job2["name"]
    ...
    # 7. Delete first job
    resp = client.delete(f"/jobs/{name1}")
    ...
    resp = client.delete(f"/jobs/{name2}")
    assert resp.status_code == 204
